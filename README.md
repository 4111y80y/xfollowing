...
关键词的选择直接影响采集到的互关用户数量和质量。推荐设置多语言关键词，覆盖更多互关用户：
...
| 回关 | 中文 | "回关"也很常见 |
| 蓝V互关 | 中文 | 明确寻找蓝V用户 |
| follow back | 英文 | 国际用户常用 |
| Follback | 英文 | follow back的缩写 |
| SDV | 缩写 | Support, Done, Verified的缩写，互关暗号 |
| フォロバ | 日文 | 日本用户互关用语 |

**设置技巧：**
1. **多语言覆盖**：添加中文、英文、日文等关键词，采集不同语言区的互关用户
2. **程序自动切换**：每30-60秒自动切换到下一个关键词搜索，无需手动操作
3. **蓝V过滤**：搜索时自动添加 `filter:blue_verified`，只显示蓝V用户的帖子
4. **双击跳转**：双击关键词可立即跳转到该关键词的Latest搜索页面
...
**新功能**
- [NEW] 休眠期间回关检查：30分钟休眠期间每60秒自动检查一个用户是否回关，充分利用等待时间
- [NEW] 未回关自动取关：休眠期间发现未回关用户会自动取消关注

**改进**
- [FIX] 修复休眠期间回关检查不执行的问题（状态标记缺失）
- [FIX] 优化启动时文件清理：只删除不影响登录的缓存文件，保留登录状态

### v1.0.8 (2026-02-05)

**新功能**
- [NEW] 自动关注看门狗：当自动关注流程停滞超过30秒时，自动检测并恢复流程
- [NEW] 流程停滞检测：每10秒检查一次流程状态，连续3次无进展时自动触发恢复

**改进**
- [FIX] 修复自动关注按钮开启但流程停止不动的问题
- [FIX] 修复回关检查与自动关注可能产生的冲突
- [FIX] 修复JS回调丢失或浏览器加载超时导致流程卡死的问题

### v1.0.7 (2026-02-04)

**新功能**
- [NEW] 关键词账号优先：关键词搜索到的账号优先于粉丝采集账号进行关注
- [NEW] 粉丝采集动态控制：有关键词账号待关注时自动暂停粉丝采集，关注完毕后自动恢复
- [NEW] 回关检查均匀分布：检查操作均匀分布在冷却时间内，避免连续取关
- [NEW] 可配置检查数量：每轮冷却期间检查1-3个用户是否回关（可配置）
- [NEW] 可配置复查间隔：已检查过的用户多少天后再次检查（1-30天可配置）
- [NEW] 详细的界面提示：所有设置控件添加详细Tooltip说明

**改进**
- [FIX] 修复session恢复弹窗：启动时自动清理会话缓存文件，不再弹出恢复窗口
- [FIX] 修复路径大小写问题：Default目录路径修正
- [FIX] 设置实时生效：修改输入框后下一回合立即生效，无需重启
- [FIX] 界面数量实时更新：粉丝面板待关注数量、已关注数量等实时动态变化
- [FIX] 禁用个人资料错误弹窗：新用户首次启动不再弹出错误对话框
- [FIX] 登录检测优化：用户登录后才创建粉丝浏览器，避免新用户困惑

**文档**
- [DOC] 添加关键词设置技巧章节，包含多语言关键词推荐（中文、英文、日文）

### v1.0.6 (2026-02-03)

**性能优化**
- [NEW] DataStorage 内存缓存：帖子列表缓存到内存，避免每次操作读取磁盘
- [NEW] O(1) 快速查询：使用 QSet 索引实现 postId 的 O(1) 查询（原来是 O(n) 磁盘读取）
- [NEW] 延迟写入：5秒定时器批量保存，大幅减少磁盘IO频率
- [NEW] 原子保存机制：写入临时文件 -> 备份原文件(.bak) -> 原子替换，确保数据安全
- [NEW] 已关注用户列表分页：每页100条，支持 [<<] [<] Page 1/40 [>] [>>] 翻页

**改进**
- [FIX] 修复大数据量（38000+帖子、3900+已关注用户）导致的界面卡顿问题
- [FIX] 程序退出时确保所有未保存数据写入磁盘
