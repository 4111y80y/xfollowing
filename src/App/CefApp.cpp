...
// Helper function to get exe directory using Windows API (works before
// QApplication)
...
// Helper: fix exit_type in Preferences to prevent session restore prompt
static void FixPreferencesExitType(const std::wstring &profileDir) {
  std::wstring prefsFile = profileDir + L"\\Preferences";
  // Read file
  std::ifstream ifs(prefsFile, std::ios::binary);
  if (!ifs.is_open())
    return;
  std::string content((std::istreambuf_iterator<char>(ifs)),
                      std::istreambuf_iterator<char>());
  ifs.close();
  // Replace exit_type from "Crashed" to "Normal"
  size_t pos = content.find("\"exit_type\":\"Crashed\"");
  if (pos != std::string::npos) {
    content.replace(pos, 22, "\"exit_type\":\"Normal\" ");
  }
  // Also handle "SessionCrashed" / empty
  pos = content.find("\"exited_cleanly\":false");
  if (pos != std::string::npos) {
    content.replace(pos, 22, "\"exited_cleanly\":true ");
  }
  // Write back
  std::ofstream ofs(prefsFile, std::ios::binary | std::ios::trunc);
  if (ofs.is_open()) {
...
void BrowserApp::OnBeforeCommandLineProcessing(
  ...
  command_line->AppendSwitch(
      "disable-features=TranslateUI,SessionRestore,TabHoverCards");
  ...
  command_line->AppendSwitch(
      "disable-features=ProfileMenuRevamp,ProfilePicker");
...
bool Initialize(int argc, char *argv[]) {
  ...
  DeleteFileW((defaultDir + L"\\Last Browser").c_str());
  ...
  // Clean Sessions directory (contains session restore data)
  DeleteDirectoryRecursive(defaultDir + L"\\Sessions");
  // Fix Preferences to mark exit as clean
  FixPreferencesExitType(defaultDir);

  // Clean up files that cause profile error dialogs (but preserve login state)
  // Note: Do NOT delete Login Data, Preferences, Secure Preferences, Local
  // State as they are required for maintaining login sessions
  ...
  DeleteFileW(
      (scannerDefaultDir + L"\\Network Action Predictor-journal").c_str());
  DeleteFileW((scannerDefaultDir + L"\\lockfile").c_str());
  DeleteFileW((scannerDefaultDir + L"\\LOCK").c_str());
  // Clean Sessions directory
  DeleteDirectoryRecursive(scannerDefaultDir + L"\\Sessions");
  // Fix Preferences to mark exit as clean
  FixPreferencesExitType(scannerDefaultDir);
...
void Shutdown() { CefShutdown(); }

void DoMessageLoopWork() { CefDoMessageLoopWork(); }
